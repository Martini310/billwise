...
class SyncAquanet(FetchSupplier, SyncSupplier):
    ...
    def login(self, session):
        ...
        login_respone = session.post('https://ebok.aquanet.pl/api/mobile/login', json=payload, verify=False)
        # Handle Login error
    ...
    def parse_invoices(self, invoices):
        ...
        for invoice in invoices.get('invoices'):
            invoice_objects.append({
                ...
                'date':               datetime.fromisoformat(invoice['issuedAt']),
                'amount':             float(invoice['value']),
                'pay_deadline':       datetime.fromisoformat(invoice['paidAt']),
                'amount_to_pay':      float(invoice.get('valueLeft', 0)),
                'user':               self.user,
...
# OLD CODE - PARSING WITH BEAUTIFULSOUP
# class SyncAquanet(FetchSupplier, SyncSupplier):
